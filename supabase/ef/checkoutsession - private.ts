...
Deno.serve(async (req) => {
    ...
    // private Supabase client for accessing the private schema
    // This is needed to access the private.customers table
    const supabaseDB = createClient(
        Deno.env.get("SUPABASE_URL") ?? "",
        Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "",
        { db: { schema: 'private' }, global: { headers: { Authorization: `Bearer ${Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")}` } } }
    );

    let stripeCustomerId = null;
    // Check if the user already exists in the customers table
    const { data: existingCustomer, error: fetchError } = await supabaseDB
        .from('customers')
        .select()
        .eq('id', user.id)
    ...
    if (!existingCustomer) {
        /*const { error: insertError } = await supabaseDB.from('customers').insert({ id: user.id, email: user.email });
        if (insertError) {
